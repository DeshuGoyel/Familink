...
export default function OpsPortal() {
  ...
  return (
    <div className="min-h-screen bg-[#020409] text-slate-200 flex flex-col md:flex-row overflow-hidden font-sans">
      ...
      <aside 
        className={cn(
          "fixed inset-y-0 left-0 z-50 bg-[#0B0E14] border-r border-slate-800 transition-all duration-300 ease-in-out transform",
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-slate-800/50 bg-slate-900/20">
            <div className={cn("flex items-center", (isSidebarOpen || isMobileMenuOpen) ? "space-x-3" : "justify-center")}>
            ...
            <button 
              ...
              className={cn(
                ...
                (isSidebarOpen || isMobileMenuOpen) ? "space-x-2 px-4" : "justify-center"
              )}
            >
              <LogOut className="w-4 h-4" />
              {(isSidebarOpen || isMobileMenuOpen) && <span className="text-xs font-semibold">Sign Out</span>}
            </button>
          </div>
        </div>
