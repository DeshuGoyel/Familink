      ...
      <main className="px-4 sm:px-6 lg:px-8 pb-28 md:pb-12 max-w-7xl mx-auto space-y-10">
        ...
        <motion.header {...fadeUp(0)} className="flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            ...
            <p className="text-muted text-sm font-medium max-w-xl">
              Securely archive your entire digital and institutional portfolio. {assets.length} items currently protected with <span className="text-secondary">${totalValue.toLocaleString()}</span> in institutional value.
            </p>
          </div>
        ...
        <motion.div {...fadeUp(0.05)} className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-base/60 pb-6">
          ...
          <div className="flex items-center gap-4">
            <div className="relative group flex-1 lg:flex-none">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-muted group-focus-within:text-brand-primary transition-colors" size={16} />
              <input 
                type="text" 
                placeholder="Search protocol records..."
                className="bg-surface border border-base rounded-2xl pl-11 pr-4 py-3 text-sm text-primary focus:border-brand-primary/50 w-full lg:w-72 transition-all placeholder:text-muted"
              />
            </div>
            <div className="flex bg-surface border border-base rounded-xl p-1">
              <button 
                onClick={() => setViewMode('grid')}
                className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-surface/80 text-brand-primary shadow-sm' : 'text-muted'}`}
              >
                <LayoutGrid size={18} />
              </button>
              <button 
                onClick={() => setViewMode('list')}
                className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-surface/80 text-brand-primary shadow-sm' : 'text-muted'}`}
              >
                <List size={18} />
              </button>
            </div>
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-8">
                    <div className="flex items-center space-x-5">
                      ...
                      <div>
                        <h3 className="text-xl font-display font-bold text-primary tracking-tight group-hover:text-brand-primary transition-colors">{asset.name}</h3>
                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-secondary mt-1">{asset.type}</p>
                      </div>
                    </div>
                    ...
                    <div className="mb-8 p-5 bg-page/80 rounded-2xl border border-base/40 group-hover:border-brand-primary/10 transition-colors shadow-inner">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-muted">Archival Projection</span>
                        </div>
                  ...
                  <div className="mt-auto pt-8 border-t border-base/60 flex justify-between items-end">
                    <div>
                      <p className="text-[9px] font-bold text-muted uppercase tracking-widest mb-1.5">Current Valuation</p>
                    ...
                    <div className="flex space-x-3">
                      ...
                      <button 
                        ...
                        className="p-3 bg-page hover:bg-red-500/10 rounded-xl text-muted hover:text-red-500 transition-all border border-base hover:border-red-500/30 shadow-inner"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {filteredAssets.map((asset) => (
                    <tr key={asset.id} className="hover:bg-surface/80 transition-colors group">
                      ...
                      <td className="px-8 py-6 text-right">
                        <button onClick={() => openEditModal(asset)} className="p-2 text-muted hover:text-brand-primary transition-colors"><Edit2 size={16}/></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {filteredAssets.length === 0 && (
          <motion.div {...fadeUp(0.2)} className="text-center py-32 bg-surface/30 rounded-[40px] border border-dashed border-base/60">
            ...
            <h3 className="text-2xl font-display font-bold text-secondary">Vault Empty</h3>
            <p className="text-secondary text-sm mt-2 max-w-xs mx-auto font-medium">No archived records found in the {activeTab} sector. Initiate your first security protocol.</p>
      ...
      <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingId(null); reset(); }} title={editingId ? "Update Protocol Record" : "New Security Protocol"}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pt-4">
          ...
          <div className="flex flex-col space-y-2">
            <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-secondary ml-1">Succession Protocol Instructions</label>
            <textarea 
              className="w-full bg-page border border-base rounded-2xl px-5 py-4 text-primary text-sm focus:border-brand-primary/50 transition-all placeholder:text-muted leading-relaxed min-h-[140px]"
              rows={4}
