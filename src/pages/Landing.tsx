...
export default function Landing() {
  ...
  useEffect(() => {
    async function loadBranding() {
      try {
        ...
        console.warn('Failed to load branding in Landing page, using defaults:', err);
      }
    }
    loadBranding();
  }, []);
  ...
  useEffect(() => {
    ...
    window.handleCTA = async () => {
      ...
      if (emailInput) {
        ...
        try {
          ...
          // Send waitlist signup to database
          await api.post('/app/waitlist', { email, name: null }, { skipAead: true });
          toast.success('Successfully joined waitlist!');
          emailInput.value = '';
        } catch (err: any) {
          console.error(err);
          toast.error(err.message || 'Failed to sign up for waitlist.');
        } finally {
    ...
    window.handleCTAWaitlistScroll = () => {
    ...
    window.handleSignupDirect = () => {
      navigate('/onboarding');
    };

    // ── SMOOTH HOVER EFFECTS ──
    const cards = container.querySelectorAll('.price-card,.step-card,.test-card') as NodeListOf<HTMLElement>;
    cards.forEach(card => {
    ...
    // ── Auth-aware CTA Swapping & Event Binding ──
    const heroActions = container.querySelector('.hero-actions') as HTMLElement | null;

    let heroBtn: Element | null = null;
    let finalCtaBtn: Element | null = null;

    if (isAuthenticated) {
      // Replace hero primary CTA with dashboard button
      if (heroActions) {
        heroBtn = heroActions.querySelector('.btn-hero');
        if (heroBtn) {
          heroBtn.innerHTML = `
            Go to Dashboard
          ...
          `;
          const clone = heroBtn.cloneNode(true) as HTMLElement;
          heroBtn.replaceWith(clone);
          heroBtn = clone;
          heroBtn.addEventListener('click', handleDashboard);
        }
      }

      // Replace Final CTA with Dashboard button
      const ctaSection = container.querySelector('.cta-section') as HTMLElement | null;
      if (ctaSection) {
        const emailFormParent = ctaSection.querySelector('.email-form')?.parentElement;
        if (emailFormParent) {
          emailFormParent.innerHTML = `
            <button class="btn-hero" id="final-cta-dashboard-btn" style="margin: 0 auto; display: inline-flex; align-items: center; gap: 8px;">
              Go to Dashboard
              <svg viewBox="0 0 24 24" style="width:16px;height:16px;stroke:currentColor;stroke-width:1.5;fill:none">
                <line x1="5" y1="12" x2="19" y2="12"/>
                <polyline points="12 5 19 12 12 19"/>
          ...
          `;
          finalCtaBtn = emailFormParent.querySelector('#final-cta-dashboard-btn');
          finalCtaBtn?.addEventListener('click', handleDashboard);
        }
      }
    } else {
      // Restore hero primary CTA button
      if (heroActions) {
        heroBtn = heroActions.querySelector('.btn-hero');
        if (heroBtn) {
      ...
      // Restore default Final CTA email form
      const ctaSection = container.querySelector('.cta-section') as HTMLElement | null;
      if (ctaSection) {
        const emailFormParent = ctaSection.querySelector('.email-form')?.parentElement;
        if (emailFormParent && !emailFormParent.querySelector('.email-form')) {
          if (branding.waitlist_enabled) {
            emailFormParent.innerHTML = `
            ...
            emailFormParent.innerHTML = `
              <div style="display:flex;justify-content:center">
                <button class="btn-hero" style="white-space:nowrap;border-radius:9px;padding:13px 32px;font-size:14px" onclick="handleSignupDirect()">Get started free</button>
              </div>
            `;
          }
        }
    ...
    priceCards.forEach((card, idx) => {
      const btn = card.querySelector('.btn-price') as HTMLElement | null;
      if (btn) {
        if (idx === 3) {
          ...
          if (isAuthenticated) {
            ...
            btn.addEventListener('click', handleDashboard);
          } else {
            if (branding.waitlist_enabled) {
              btn.textContent = idx === 1 ? 'Join Waitlist' : 'Get started';
              ...
              if (idx === 1) {
                btn.textContent = 'Claim founding price';
              } else {
    ...
    return () => {
      ...
      if (isAuthenticated) {
        heroBtn?.removeEventListener('click', handleDashboard);
        if (finalCtaBtn) {
          finalCtaBtn.removeEventListener('click', handleDashboard);
        }
      } else {
      ...
      priceCards.forEach((card, idx) => {
        ...
        if (btn) {
          if (idx === 3) {
            ...
            btn.removeEventListener('click', isAuthenticated ? handleDashboard : handleSignup);
          }
        }
      });
      footerLinks.forEach(link => {
  ...
  }, [isAuthenticated, navigate, branding.waitlist_enabled]);
  ...
  // Dynamically do string replacement in rawHtml based on loaded branding state
  let processedHtml = rawHtml;

  // Replace Brand Name
  processedHtml = processedHtml.replace(/Transfer Legacy/g, branding.brand_name);

  // Replace Logo if URL exists
  if (branding.logo_url) {
    // Replace sidebar preview logo SVG
    processedHtml = processedHtml.replace(
      /<div style="width:20px;height:20px;background:rgba\(249,115,22,\.12\);border:1px solid rgba\(249,115,22,\.3\);border-radius:5px;display:flex;align-items:center;justify-content:center"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var\(--or\)" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"\/><\/svg><\/div>/,
      `<div style="width:20px;height:20px;background:rgba(249,115,22,.12);border:1px solid rgba(249,115,22,.3);border-radius:5px;display:flex;align-items:center;justify-content:center;overflow:hidden"><img src="${branding.logo_url}" alt="Logo" style="width:100%;height:100%;object-fit:cover;padding:2px" /></div>`
    );
    // Replace footer logo SVG
    processedHtml = processedHtml.replace(
      /<div class="nav-logo"><svg viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"\/><\/svg><\/div>/,
      `<div class="nav-logo" style="overflow:hidden;display:flex;align-items:center;justify-content:center"><img src="${branding.logo_url}" alt="Logo" style="width:100%;height:100%;object-fit:contain" /></div>`
    );
  }

  // Handle Waitlist Enabled / Disabled CTAs
  if (!branding.waitlist_enabled) {
    // Replace hero actions primary button text
    processedHtml = processedHtml.replace(
      /<button class="btn-hero">\s*Protect your family[\s\S]*?<\/button>/,
      `<button class="btn-hero" onclick="handleSignupDirect()">
        Protect your family
        <svg viewBox="0 0 24 24"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
      </button>`
    );
    // Replace email form with direct onboarding link
    processedHtml = processedHtml.replace(
      /<div class="email-form">[\s\S]*?<\/div>/,
      `<div style="display:flex;justify-content:center">
        <button class="btn-hero" style="white-space:nowrap;border-radius:9px;padding:13px 32px;font-size:14px" onclick="handleSignupDirect()">Get started free</button>
      </div>`
    );
  } else {
    // If waitlist is active, hero CTA should scroll down to waitlist form
    processedHtml = processedHtml.replace(
      /<button class="btn-hero">\s*Protect your family[\s\S]*?<\/button>/,
      `<button class="btn-hero" onclick="handleCTAWaitlistScroll()">
        Join Early Access
        <svg viewBox="0 0 24 24"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
      </button>`
    );
  ...
  const rawHtmlWithoutNavbar = processedHtml.replace(/<!-- ══════════════ NAV ══════════════ -->[\s\S]*?<\/nav>/, '');
...
declare global {
  interface Window {
    ...
    handleCTA: () => void;
    handleCTAWaitlistScroll: () => void;
    handleSignupDirect: () => void;
  }
}
